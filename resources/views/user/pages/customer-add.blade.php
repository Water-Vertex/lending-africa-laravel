{{-- resources/views/user/pages/customer-add.blade.php --}}
            ...
            <form method="POST" action="{{ route('staff.customer.store') }}" enctype="multipart/form-data" id="customer-form" class="space-y-8">
                ...
                <div id="step-1">
                    ...
                    <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                        <a href="{{ url()->previous() }}" class="btn-outline-white !text-gray-700 !border-gray-300 px-4 py-2 rounded-xl text-sm font-medium">
                            Cancel
                        </a>
...
{{-- Inline script — NOT dependent on @push/@stack, guaranteed to run --}}
<script>
    (function () {
        function initCustomerForm() {
            ...
            if (!wrapper || !template || !addBtn || !form) {
                console.error('Customer form: required elements not found on page.');
                return;
            }
            ...
            // one empty document row visible by default
            addDocumentRow();
            ...
            // =====================================================
            // Customer Type -> Business Section toggle + Loan Product filter
            // =====================================================
            var customerTypeSelect = document.getElementById('customer_type');
            var businessSection    = document.getElementById('business-info-section');
            var loanProductSelect  = document.getElementById('loan_product_id');
...
function filterLoanProducts() {
    ...
    // Agar is customer type ke liye koi product available nahi hai
    ...
    updateLoanAmountLimits();
            ...
            function updateLoanAmountLimits() {
                ...
                if (selected && selected.dataset.min) {
                    ...
                    if (!durationInput.value && selected.dataset.duration) {
            ...
            // initial state (page load / validation-error reload par bhi sahi state)
            toggleBusinessSection();
            filterLoanProducts();

            // =====================================================
            // Step navigation (Next / Back) — same form, no page reload
            // =====================================================
            ...
            @if ($errors->any())
                // Agar validation error step-2 (ya business) fields me hai, to
                // page reload hone par seedha sahi step dikhao.
