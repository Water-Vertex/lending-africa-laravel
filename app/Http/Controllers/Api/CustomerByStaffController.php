...
class CustomerByStaffController extends Controller
{
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        // }
            // 4) Documents — using $request->input()/$request->file() directly
// instead of $validated['documents'][$index], since relying on the
// validated array's index can silently mismatch if any entry was
// filtered/reindexed. Logging added temporarily to diagnose.
$documentFiles = $request->file('documents', []);

Log::info('update: raw documents input = ' . json_encode($request->input('documents')));
Log::info('update: documents file entries = ' . count($documentFiles));

foreach ($documentFiles as $index => $docFiles) {
    $file = $docFiles['file'] ?? null;

    Log::info("STORE: document[{$index}] file present = " . ($file ? 'YES (' . $file->getClientOriginalName() . ')' : 'NO'));
    ...
    $customer->documents()->create([
        ...
        'file_path'            => $path,
        'verification_status'  => $request->input("documents.{$index}.verification_status", 'pending'),
    ]);

    Log::info("STORE: document[{$index}] saved to {$path}");
}
...
Log::info('STORE: raw documents input = ' . json_encode($request->input('documents')));
Log::info('STORE: documents file entries = ' . count($documentFiles));
