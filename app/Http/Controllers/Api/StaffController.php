...
class StaffController extends Controller
{
     ...
     */
    public function index(Request $request) // 👈 CHANGED: added Request $request
    {
        try {
            $admin = $request->user(); // 👈 NEW
            $query = Staff::with('bank')->orderBy('id', 'desc'); // 👈 CHANGED

            // 👈 NEW BLOCK: scope to own staff only if Branch Manager
            if ($admin && $admin->role && $admin->role->name === 'Branch Manager') {
                $query->where('created_by', $admin->id);
            }

            $staff = $query->get(); // 👈 CHANGED
     ...
     *
     * Branch Manager => bank_id/branch_name are FORCED from their own account,
     *                   client-sent values are always ignored.
     * Admin          => bank_id required from request, branch_name manual text.
     *
    ...
    {
        $admin = $request->user(); // 👈 NEW
        $isBranchManager = $admin && $admin->role && $admin->role->name === 'Branch Manager'; // 👈 NEW

        // 👈 CHANGED: validation rules now built conditionally
        $rules = [
        ...
        if ($isBranchManager) {
            // 👈 NEW: Branch Manager must already have a bank/branch assigned on their own account
            if (!$admin->bank_id) {
                return response()->json([
                    'success' => false,
                    'message' => 'Your account has no bank/branch assigned. Please contact admin.'
                ], 422);
            }
            // Note: bank_id / branch_name are intentionally NOT in $rules here.
            // We never trust request input for these fields when a Branch Manager submits.
        } else {
            // Admin (or other full-access roles): bank_id required, branch_name optional manual text
        ...
        try {
            // 👈 NEW BLOCK: force bank/branch based on role, never trust client input for Branch Manager
            if ($isBranchManager) {
            ...
            $bank = Bank::findOrFail($bankId); // 👈 CHANGED: uses $bankId instead of $request->bank_id
            ...
            $staff = Staff::create([
                ...
                'bank_id'       => $bankId,       // 👈 CHANGED
                'branch_name'   => $branchName,   // 👈 CHANGED
                ...
                'created_by'    => $admin->id, // 👈 NEW: track who created this staff
            ]);
     ...
     * Branch Manager can only view staff THEY created.
     */
    public function show(Request $request, $id) // 👈 CHANGED: added Request $request
    {
        try {
            $admin = $request->user(); // 👈 NEW
            $staff = Staff::with('bank')->findOrFail($id);

            // 👈 NEW BLOCK: ownership check
     ...
     * Branch Manager => can only update staff THEY created,
     *                   and can NEVER change bank_id/branch_name.
     * Admin          => full access, can update bank_id/branch_name.
    ...
    public function update(Request $request, $id)
    {
        ...
        // 👈 NEW BLOCK: fetch staff first + ownership check before validating input
        try {
        ...
        // 👈 CHANGED: validation rules built conditionally
        $rules = [
            'first_name'          => 'required|string|max:255',
            'last_name'           => 'required|string|max:255',
            'email'               => [
        ...
        if (!$isBranchManager) {
            // 👈 NEW: only Admin can send/change bank_id and branch_name
            $rules['bank_id']     = 'required|exists:banks,id';
            $rules['branch_name'] = 'nullable|string|max:255';
        }
        ...
        try {
            ...
            // 👈 NEW BLOCK: Branch Manager can never touch bank_id/branch_name, even on update
            if (!$isBranchManager) {
     ...
     * Branch Manager can only delete staff THEY created.
     */
    public function destroy(Request $request, $id) // 👈 CHANGED: added Request $request
    {
        try {
            $admin = $request->user(); // 👈 NEW
            $staff = Staff::findOrFail($id);
